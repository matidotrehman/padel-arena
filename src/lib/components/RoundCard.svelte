<script>
  ...
  import { ChevronRight, MoonStar, Ban } from '@lucide/svelte';
...
<div class="glass rounded-[var(--radius-md)] p-3 space-y-2.5 transition-[border-color,box-shadow] duration-200"
     style={isNext
       ? `border-color:color-mix(in srgb, var(--accent-fg) 50%, transparent);box-shadow:var(--shadow-card-accent);`
  ...
  <div class="flex items-center justify-between gap-2">
    <div class="flex items-center gap-1.5">
      ...
      {#if isNext}<span class="chip neon-text inline-flex items-center gap-1" style="background:color-mix(in srgb, var(--accent-fg) 16%, transparent);"><ChevronRight size={12} strokeWidth={2.5} /> Up next</span>{/if}
    </div>
    <div class="flex items-center gap-2">
      {#if resting.length}
        <span class="text-[11px] tx-faint truncate inline-flex items-center gap-1"><MoonStar size={12} strokeWidth={2.25} /> {resting.map((p) => p.name).join(', ')}</span>
      {/if}
      {#if hasScore}
        <button class="chip tx-muted hover:text-hot transition-transform duration-150 active:scale-95"
                style="background:color-mix(in srgb, var(--tx) 7%, transparent);transition-timing-function:var(--ease-spring);"
                onclick={markNotPlayed}><Ban size={11} strokeWidth={2.25} /> Not played</button>
      {:else}
  ...
  <div class="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
    ...
    <div class="flex flex-col gap-1 {aWon ? '' : done ? 'opacity-50' : ''}">
      ...
      {/each}
      <input class="input mono text-center text-lg font-bold py-1.5 mt-1 {aWon ? 'accent-el' : ''}" type="number" min="0" inputmode="numeric"
    ...
    <div class="flex flex-col gap-1 {bWon ? '' : done ? 'opacity-50' : ''}">
      {#each teamB as p}
        <div class="flex items-center gap-1.5"><Avatar player={p} size={26} /><span class="text-sm truncate">{p.name}</span></div>
      {/each}
      <input class="input mono text-center text-lg font-bold py-1.5 mt-1 {bWon ? 'accent-el' : ''}" type="number" min="0" inputmode="numeric"
             placeholder="0" value={round.scoreB ?? ''} oninput={(e) => set('b', e.target.value)}
             style={bWon ? 'border-color:var(--accent-fg);color:var(--accent-fg);' : ''} />
    </div>
  </div>
</div>
