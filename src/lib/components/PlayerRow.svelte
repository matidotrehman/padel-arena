...
<button
  ...
  class="glass relative overflow-hidden w-full text-left transition active:scale-[0.99] {isTop
    ? 'rounded-[28px] px-4 pt-4 pb-3.5 flex flex-col gap-3'
    : 'rounded-3xl px-3 py-3 flex flex-col gap-1.5'}"
  style={isTop
...
>
  {#if isTop}
    ...
    <div class="absolute right-3 top-3 text-[10px] font-bold uppercase tracking-[0.18em] h-display" style="color:var(--color-gold);">Leading</div>

    <div class="flex items-center gap-3.5 relative z-10">
      <div class="relative shrink-0">
        <Avatar {player} size={58} />
        <span class="absolute -bottom-1.5 -right-1.5 text-xl leading-none drop-shadow-[0_2px_3px_rgba(0,0,0,0.5)]">🥇</span>
      </div>
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-1.5">
          <span class="h-display font-extrabold truncate tx text-[20px] leading-tight">{player.name}</span>
        ...
        <div class="text-[11px] tx-muted flex flex-wrap items-center gap-x-1.5 gap-y-0.5 mt-1 mono font-semibold">
          ...
          <span class="tx-faint">·</span>
          <span class={diff >= 0 ? 'neon-text' : ''} style={diff >= 0 ? '' : 'color:#ff5e3a;'}>{diff >= 0 ? '+' : ''}{diff}pd</span>
        </div>
      </div>
      <div class="text-right shrink-0">
        <div class="mono font-extrabold leading-none text-[34px]" style="color:var(--color-gold);">
          {byWinrate ? wr : avg}{#if byWinrate}<span class="text-base align-top">%</span>{/if}
        </div>
