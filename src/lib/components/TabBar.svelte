...
<nav class="fixed bottom-0 inset-x-0 z-40 px-3 pt-2"
  ...
  <div class="max-w-lg mx-auto grid grid-cols-7 gap-0.5 glass rounded-[22px] p-1.5">
    {#each tabs as t}
      {@const on = active === t.id}
      <button class="relative flex flex-col items-center gap-1 py-2 rounded-2xl transition-all duration-200"
              style={on
                ? 'background:linear-gradient(160deg,color-mix(in srgb, var(--color-neon-green) 16%, transparent),rgba(47,240,214,0.08));box-shadow:inset 0 0 0 1px color-mix(in srgb, var(--color-neon-green) 30%, transparent);'
                : ''}
              onclick={() => (active = t.id)}>
